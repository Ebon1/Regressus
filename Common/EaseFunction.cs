...
namespace Regressus.Common
{
    public abstract class EaseFunction
	{
		...
		public virtual float Ease(float time) { throw new NotImplementedException(); }
	}

	public class PolynomialEase : EaseFunction
	{
		private Func<float, float> function;
		public PolynomialEase(Func<float, float> func) { function = func; }

		public override float Ease(float time) => function(time);
	}

	public class EaseBuilder : EaseFunction
	{
		private List<EasePoint> points;

		public EaseBuilder() { points = new List<EasePoint>(); }
		...
		public void AddPoint(Vector2 vector, EaseFunction function)
		{
			...
			EasePoint last = points[points.Count - 1];
		...
		public override float Ease(float time)
		{
			...
			{
				usePoint = points[i];
				if (time <= usePoint.Point.X) { break; }

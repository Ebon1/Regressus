...
namespace Regressus.Content.Items.Dev.Decrypt
{
    ...
    public class DecryptItemP2 : HeldSword
    {
        ...
        {
            Projectile.localAI[0]++;
            Texture2D glow = ModContent.Request<Texture2D>("Regressus/Content/Items/Dev/Decrypt/DecryptItemP2_Glow").Value;
            Player player = Main.player[Projectile.owner];
            RegreUtils.Reload(Main.spriteBatch, BlendState.Additive);
            Texture2D slash = ModContent.Request<Texture2D>("Regressus/Extras/Extras2/slash_02").Value;
            float mult = Lerp(Utils.GetLerpValue(0f, SwingTime, Projectile.timeLeft));
    ...
    public class DecryptItemP : ModProjectile
    {
        ...
        public override string Texture => "Regressus/Content/Items/Dev/Decrypt/_DecryptItem";
        float a, b, c;
        ...
        {
            Projectile.localAI[0]++;
            Texture2D glow = ModContent.Request<Texture2D>("Regressus/Content/Items/Dev/Decrypt/DecryptItemP_Glow").Value;
            Player player = Main.player[Projectile.owner];
            RegreUtils.Reload(Main.spriteBatch, BlendState.Additive);
            var fadeMult = 1f / ProjectileID.Sets.TrailCacheLength[Projectile.type];
            for (int i = 0; i < ProjectileID.Sets.TrailCacheLength[Projectile.type]; i++)
    ...
    {
        public override string Texture => "Regressus/Content/Projectiles/Misc/OracleEnergyOrb";
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Energy Orb");
            Main.projFrames[Projectile.type] = 4;

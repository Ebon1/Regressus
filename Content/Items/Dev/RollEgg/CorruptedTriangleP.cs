...
namespace Regressus.Content.Items.Dev.RollEgg
{
    public class CorruptedTriangleP1 : ModProjectile
    {
        ...
        {
            for (int i = 0; i < 5; i++)
                Dust.NewDustDirect(Projectile.Center, 5, 5, ModContent.DustType<RollParticle>(), -Projectile.velocity.RotatedBy(Math.PI / (Projectile.localAI[0] / 5)).X / 5, -Projectile.velocity.RotatedBy(Math.PI / (Projectile.localAI[0] / 5)).Y / 5);
        }

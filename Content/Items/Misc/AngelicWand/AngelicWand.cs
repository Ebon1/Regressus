...
namespace Regressus.Content.Items.Misc.AngelicWand
{
    public class AngelicWand : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (attacks >= 5)
            {
                ...
                vel.Normalize();
                Projectile.NewProjectile(source, position + velocity, vel, ModContent.ProjectileType<LuminaryPF>(), damage * 2, knockback, player.whoAmI);
                attacks = 0;
            }
            else
                for (int i = 0; i < 2; i++)

...
namespace Regressus.NPCs.Bosses.Oracle
{
    ...
    public class OracleBoss : ModNPC
    {
        ...
        {
            Vector2 drawOrigin = new(ModContent.Request<Texture2D>("Regressus/NPCs/Bosses/Oracle/OracleBoss").Value.Width * 0.5f, NPC.height * 0.5f);
        ...
        public override void AI()
        {
            ...
            if (AIState == FlyAndShootSpikes)
            {
                ...
                if (haveDonePhase2Transition && NPC.life <= NPC.lifeMax / 2)
                {
                    ...
                    Vector2 pos = new(player.position.X + 300 - funny, player.position.Y - 400 - (funny / 10));
                    Vector2 target = pos;
                ...
                else
                {
                    ...
                    Vector2 pos = new(player.position.X + 300 - funny, player.position.Y - 300 - (funny / 10));
                    Vector2 target = pos;
                    Vector2 moveTo = target - NPC.Center;
                    NPC.velocity = (moveTo) * 0.0505f;
            ...
            else if (AIState == CrystalRain)
            {
                ...
                if (haveDonePhase2Transition && NPC.life <= NPC.lifeMax / 2)
                {
                    Vector2 pos = new(player.position.X, player.position.Y - 450);
                    Vector2 target = pos;
                    Vector2 moveTo = target - NPC.Center;
                    NPC.velocity = (moveTo) * 0.0505f;
                    ...
                    if (++AITimer2 >= 45)
                    {
                        ...
                        {
                            Vector2 vector17 = new(player.Center.X - num53 / 2f + num54 * (float)num56, player.Center.Y - 1300f);
                ...
                {
                    Vector2 pos = new(player.position.X, player.position.Y - 300);
                    Vector2 target = pos;
                    Vector2 moveTo = target - NPC.Center;
                    NPC.velocity = (moveTo) * 0.0505f;
                    ...
                    if (++AITimer2 >= 30)
                    {
                        for (int i = 0; i <= 5; i++)
                        {
                            Projectile projectile = Main.projectile[Projectile.NewProjectile(NPC.GetProjectileSpawnSource(), realNpcCenter, 6.5f * Utils.RotatedBy(NPC.DirectionTo(new Vector2(0, -1)), (double)(MathHelper.ToRadians(Main.rand.NextFloat(15f, 55f)) * (float)i), default), ModContent.ProjectileType<CrystalProjectilesFromTheHitGameAmongUs>(), 20, 1f, Main.myPlayer)];
                            projectile.tileCollide = true;
                            projectile.timeLeft = 230;
                        }
                        AITimer2 = 0;
            ...
            else if (AIState == SummonPortals)
            {
                ...
                if (haveDonePhase2Transition && NPC.life <= NPC.lifeMax / 2)
                {
                    Vector2 impostorlookingkindasusngl = new(Main.screenPosition.X + Main.screenWidth * Main.rand.NextFloat(), Main.screenPosition.Y + Main.screenHeight * Main.rand.NextFloat());
                    if (AITimer <= 100)
                    {
                        if (++AITimer2 >= 15)
                        {
                ...
                else
                {
                    ...
                    if (AITimer2 == 25)
                    {
                        Vector2 sussyBaka = new(Main.screenPosition.X, Main.screenPosition.Y);
                    ...
                    if (AITimer2 == 50)
                    {
                        Vector2 sussyBaka = new(Main.screenPosition.X + Main.screenWidth, Main.screenPosition.Y);
                        Projectile projectile = Main.projectile[Projectile.NewProjectile(NPC.GetProjectileSpawnSource(), sussyBaka, Vector2.Zero, ModContent.ProjectileType<Portale>(), 0, 1f, Main.myPlayer)];
                    }
                    if (AITimer2 == 75)
                    {
                        Vector2 sussyBaka = new(Main.screenPosition.X + Main.screenWidth, Main.screenPosition.Y + Main.screenHeight);
                        Projectile projectile = Main.projectile[Projectile.NewProjectile(NPC.GetProjectileSpawnSource(), sussyBaka, Vector2.Zero, ModContent.ProjectileType<Portale>(), 0, 1f, Main.myPlayer)];
                    }
                    if (AITimer2 == 100)
                    {
                        Vector2 sussyBaka = new(Main.screenPosition.X, Main.screenPosition.Y + Main.screenHeight);
                        Projectile projectile = Main.projectile[Projectile.NewProjectile(NPC.GetProjectileSpawnSource(), sussyBaka, Vector2.Zero, ModContent.ProjectileType<Portale>(), 0, 1f, Main.myPlayer)];
                    }
                }
                if (AITimer >= 180)
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            {
                Dust.NewDust(NPC.position, NPC.width, NPC.height, dustthing, hitDirection, -1f, 0, default, 1f);
            }
        }

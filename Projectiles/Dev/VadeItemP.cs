...
namespace Regressus.Projectiles.Dev
{
    ...
    public class VadeItemP3 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                Projectile.rotation += MathHelper.ToRadians(10f);
                Projectile.velocity = (targetCenter - Projectile.Center) * 0.028f;
            }
            else
            {
        ...
        public override void PostDraw(Color lightColor)
        {
            Player player = Main.player[Projectile.owner];
            Texture2D texture = TextureAssets.Projectile[Projectile.type].Value;
            Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, null, Color.White * 0.5f, Projectile.rotation + (Projectile.direction == -1 ? 0 : MathHelper.PiOver2 * 3), texture.Size() / 2, Projectile.scale, Projectile.direction == -1 ? SpriteEffects.None : SpriteEffects.FlipVertically, 0f);
        }
        public override bool PreDraw(ref Color lightColor)
        {
            Texture2D glow = ModContent.Request<Texture2D>("Regressus/Projectiles/Dev/VadeItemP_Glow").Value;
            ...
            //Vector2 pos = player.Center + Projectile.velocity * ((holdOffset * 0.75f) - mult * (holdOffset * 0.75f));
            //Main.spriteBatch.Draw(slash, pos - Main.screenPosition, null, Main.DiscoColor * alpha, Projectile.velocity.ToRotation() - MathHelper.PiOver2, slash.Size() / 2, Projectile.scale / 1.95f, SpriteEffects.None, 0f);
            Main.spriteBatch.Draw(glow, Projectile.Center - Main.screenPosition, new Rectangle(0, 0, glow.Width, glow.Height), Main.DiscoColor, Projectile.rotation + (Projectile.direction == -1 ? 0 : MathHelper.PiOver2 * 3), glow.Size() / 2, Projectile.scale * 1.05f, Projectile.direction == -1 ? SpriteEffects.None : SpriteEffects.FlipVertically, 0f);
            RegreUtils.Reload(Main.spriteBatch, BlendState.AlphaBlend);
            return false;
            /*Projectile.localAI[0]++;
            RegreUtils.Reload(Main.spriteBatch, BlendState.Additive);

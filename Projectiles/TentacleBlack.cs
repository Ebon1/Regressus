...
{
    internal class TentacleBlack : ModProjectile
    {
        public override string Texture => "Regressus/Extras/Tentacle";
        ...
        {
            List<VertexTest> bars = new List<VertexTest>();
            for (int i = 1; i < len; i++)
            {
                ...
                bars.Add(new VertexTest(v1 + w * normaldir, Color.Blue, new Vector3(factor, 0f, 0f)));
                bars.Add(new VertexTest(v1 - w * normaldir, Color.Blue, new Vector3(factor, 1f, 0f)));
            }
            if (bars.Count > 2)
            {
                ...
                Regressus.TentacleBlack.Parameters[0].SetValue(model * projection);
                Regressus.TentacleBlack.CurrentTechnique.Passes[0]
                    .Apply();
                ...
                ((Game)Main.instance).GraphicsDevice.DrawUserPrimitives<VertexTest>((PrimitiveType)1, bars.ToArray(), 0, bars.Count - 2);
                Main.spriteBatch.End();
    ...
    internal class SmolTentacleBlack : ModProjectile
    {
        public override string Texture => "Regressus/Extras/laser";
        private List<float> rots;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 1; i < len; i++)
            {
                ...
                ((Vector2)(normaldir)).Normalize();
                float w = (Projectile.ai[0] == 400 ? 28 : 32f) * MathHelper.SmoothStep(0.8f, 0.1f, factor);
                bars.Add(new VertexTest(v1 + w * normaldir, Color.White, new Vector3(factor, 0f, 0f)));
                bars.Add(new VertexTest(v1 - w * normaldir, Color.White, new Vector3(factor, 1f, 0f)));
            }
            if (bars.Count > 2)
            {
                Main.spriteBatch.End();
